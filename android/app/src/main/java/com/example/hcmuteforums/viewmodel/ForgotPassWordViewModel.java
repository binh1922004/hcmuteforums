...
public class ForgotPassWordViewModel extends ViewModel {
    ...
    private MutableLiveData<Event<Boolean>> sendOtpResetPWError = new MutableLiveData<>();
    private MutableLiveData<Event<Boolean>> sendOtpResetPWResponse = new MutableLiveData<>();
    ...
    public void sendOtpResetPassword(String email, String username){
        isLoading.setValue(true);
        otpRepository.sendOtpResetPasword(email, username, new Callback<ApiResponse<Boolean>>() {
            @Override
            public void onResponse(Call<ApiResponse<Boolean>> call, Response<ApiResponse<Boolean>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ApiResponse<Boolean> apiRes = response.body();
                    if (apiRes.getResult()) {
                        sendOtpResetPWResponse.setValue(new Event<>(true));
                    } else {
                        sendOtpResetPWError.setValue(new Event<>(true));
                    }
                }else {
    ...
    public MutableLiveData<Event<Boolean>> getSendOtpResetPWError() {
        return sendOtpResetPWError;
    }

    public MutableLiveData<Event<Boolean>> getSendOtpResetPWResponse() {
        return sendOtpResetPWResponse;
    }
